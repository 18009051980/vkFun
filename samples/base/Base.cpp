...
bool Base::tick(float delta) 
{
    return GlfwApp::tick(delta);
}

void Base::loadModel(const char* filename)
{
    ObjLoader objLoader;
...
void Base::createVertexBuffer(const std::vector<MyVertex>& vertices) 
{
    //创建一个临时的缓冲区，用于存储顶点数据
...
void Base::createIndexBuffer(const std::vector<uint32_t>& indices)
{
...
void Base::allocateDescriptorSets()
{
...
void Base::createGraphicsPipeline()
{
    ...
    multisampling.rasterizationSamples = m_maxSampleCount;
...
void Base::updateUniformBuffer()
{
    ...
    UBO ubo = {
        .model = glm::rotate(glm::mat4(1.f), time * glm::radians(30.f), glm::vec3(0.f, 0.f, 1.f)),
        .view = glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f)),
        .proj = glm::perspective(glm::radians(45.0f), m_extent.width / (float) m_extent.height, 0.1f, 10.0f)
    };
    ubo.proj[1][1] *= -1.f;
...
void Base::createTextureImage(const char* filename, VkImage &image, VkDeviceMemory& imageMemory, int& width, int& height)
{
    int channels;
...
void Base::createDepthImage()
{
    VkFormat depthFormat = VK_FORMAT_D32_SFLOAT;
    createImage(m_extent.width, m_extent.height, 1, m_maxSampleCount, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT , VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_depthImage, m_depthImageMemory);
...
void Base::createColorResources()
{
    createImage(m_extent.width, m_extent.height, 1, m_maxSampleCount, m_format, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_colorImage, m_colorImageMemory);

    m_colorImageView = createImageView(m_colorImage, m_format, VK_IMAGE_ASPECT_COLOR_BIT, 1);
}

void Base::cleanupColorResources()
{
